...
assert.equal(brief.version,'3.3.0');
assert.equal(brief.momentum.value,46);
assert.equal(brief.forecast.label,'On track');
assert.equal(brief.plan.length,3);
assert.equal(brief.closeout.reviewed,false);
assert.equal(brief.tomorrow.title,'Define the next action for Tournament preparation');
...
assert(brief.closeout.carryover.some(item=>item.type==='Overdue task'));
state.reviews=[{date:new Date().toISOString().slice(0,10),wins:'Executed plan',tomorrow:'Start recovery walk'}];
brief=mc.briefing(state,new Date('2026-07-19T20:00:00'));
assert.equal(brief.closeout.status,'Closed');
assert.equal(brief.tomorrow.title,'Start recovery walk');
